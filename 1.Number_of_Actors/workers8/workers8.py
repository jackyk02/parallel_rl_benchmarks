...
# Python class for reactor _workers8_main
class __workers8_main:
...
# Instantiate classes
workers8_main_lf = [None] * 1
workers8_client_lf = [None] * 8
workers8_server_lf = [None] * 1
workers8_delay_lf = [None] * 1
# Start initializing workers8 of class _workers8_main
for workers8_main_i in range(1):
    bank_index = workers8_main_i
    workers8_main_lf[0] = __workers8_main(
        _bank_index = 0,
    )
    # Start initializing workers8.client of class _clientreactor
    for workers8_client_i in range(8):
        bank_index = workers8_client_i
        workers8_client_lf[workers8_client_i] = __clientreactor(
            _bank_index = workers8_client_i,
        )
    # Start initializing workers8.server of class _serverreactor
    for workers8_server_i in range(1):
        bank_index = workers8_server_i
        workers8_server_lf[0] = __serverreactor(
            _bank_index = 0,
        )
    # Start initializing workers8.delay of class __lf_gendelay_0
    for workers8_delay_i in range(1):
        bank_index = workers8_delay_i
        workers8_delay_lf[0] = ___lf_gendelay_0(
            _bank_index = workers8_delay_i,
            _delay=0,
        )
