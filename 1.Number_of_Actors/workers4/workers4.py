...
# Python class for reactor _workers4_main
class __workers4_main:
...
# Instantiate classes
workers4_main_lf = [None] * 1
workers4_client_lf = [None] * 4
workers4_server_lf = [None] * 1
workers4_delay_lf = [None] * 1
# Start initializing workers4 of class _workers4_main
for workers4_main_i in range(1):
    bank_index = workers4_main_i
    workers4_main_lf[0] = __workers4_main(
        _bank_index = 0,
    )
    # Start initializing workers4.client of class _clientreactor
    for workers4_client_i in range(4):
        bank_index = workers4_client_i
        workers4_client_lf[workers4_client_i] = __clientreactor(
            _bank_index = workers4_client_i,
        )
    # Start initializing workers4.server of class _serverreactor
    for workers4_server_i in range(1):
        bank_index = workers4_server_i
        workers4_server_lf[0] = __serverreactor(
            _bank_index = 0,
        )
    # Start initializing workers4.delay of class __lf_gendelay_0
    for workers4_delay_i in range(1):
        bank_index = workers4_delay_i
        workers4_delay_lf[0] = ___lf_gendelay_0(
            _bank_index = workers4_delay_i,
            _delay=0,
        )
