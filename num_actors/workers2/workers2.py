...
# Python class for reactor _workers2_main
class __workers2_main:
...
# Instantiate classes
workers2_main_lf = [None] * 1
workers2_client_lf = [None] * 2
workers2_server_lf = [None] * 1
workers2_delay_lf = [None] * 1
# Start initializing workers2 of class _workers2_main
for workers2_main_i in range(1):
    bank_index = workers2_main_i
    workers2_main_lf[0] = __workers2_main(
        _bank_index = 0,
    )
    # Start initializing workers2.client of class _clientreactor
    for workers2_client_i in range(2):
        bank_index = workers2_client_i
        workers2_client_lf[workers2_client_i] = __clientreactor(
            _bank_index = workers2_client_i,
        )
    # Start initializing workers2.server of class _serverreactor
    for workers2_server_i in range(1):
        bank_index = workers2_server_i
        workers2_server_lf[0] = __serverreactor(
            _bank_index = 0,
        )
    # Start initializing workers2.delay of class __lf_gendelay_0
    for workers2_delay_i in range(1):
        bank_index = workers2_delay_i
        workers2_delay_lf[0] = ___lf_gendelay_0(
            _bank_index = workers2_delay_i,
            _delay=0,
        )
